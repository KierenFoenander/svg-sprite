...
`@nuxtjs/mhj-svg-sprite@v1` is compatible with [Nuxt 3](https://nuxt.com/). If you're looking for the previous version of this module, check out [the previous version](https://github.com/nuxt-community/svg-sprite-module/tree/0.x/).

::

Add `@nuxtjs/mhj-svg-sprite` dependency to your project:

::code-group

```bash [Yarn]
yarn add --dev @nuxtjs/mhj-svg-sprite
```

```bash [NPM]
npm install --save-dev @nuxtjs/mhj-svg-sprite
```

::

Then, add `@nuxtjs/mhj-svg-sprite` to the `modules` section of your `nuxt.config.js`
...
[![Edit @nuxtjs/mhj-svg-sprite](https://codesandbox.io/static/img/play-codesandbox.svg)](https://codesandbox.io/s/github/nuxt-community/svg-sprite-module/tree/master/?fontsize=14&hidenavigation=1&theme=dark)
